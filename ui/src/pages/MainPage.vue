...
function setInput(inputRef?: PlRef) {
  ...
  if (inputRef)
    app.model.args.title = app.model.outputs.countsOptions?.find(
      (o: { ref: PlRef; label: string }) => plRefsEqual(o.ref, inputRef),
    )?.label;
...
function createDefaultOptions(
  ...
  if (coord1Index === -1 || coord2Index === -1) return undefined;
    ...
    >
      ...
      <template #settingsSlot>
        ...
        <PlAccordionSection :style="{ width: '320px' }" label="ADVANCED SETTINGS">
          <PlRow>
            <PlNumberField v-model="app.model.args.nPCs" label="N PCs" :min-value="20" :step="1">
              <template #tooltip>
                <div>
                  <strong>Number of Principal Components (PCs)</strong><br />
                  Controls how many principal components are used for UMAP and t-SNE
                  calculations.<br /><br />
                  ...
                  <strong>Effect:</strong> Higher values capture more variance but increase
                  computation time.
                </div>
              </template>
            </PlNumberField>
            ...
            >
              <template #tooltip>
                <div>
                  ...
                  <strong>Effect:</strong> Lower values preserve local structure, higher values
                  preserve global relationships.
                </div>
              </template>
            </PlNumberField>
          </PlRow>
          <PlCheckbox v-model="app.model.args.hvgEnabled">
            ...
            <PlTooltip class="info">
              <template #tooltip>
                Restricts the analysis to the most informative biological variation by using only
                the top N highly variable genes (HVG), which also significantly speeds up the
                computation.
              </template>
            </PlTooltip>
          </PlCheckbox>
          ...
          <PlAlert v-if="app.model.args.nPCs > 20 && app.model.args.nPCs < 30" type="warn">
            ...
            The selected number of PCs ({{ app.model.args.nPCs }}) is below the recommended range
            (30-50). This may result in loss of important biological variation. Consider increasing
            to 30-50 for better results.
          </PlAlert>
          <PlAlert v-if="app.model.args.nPCs > 50" type="warn">
            <template #title>High PC Count</template>
            The selected number of PCs ({{ app.model.args.nPCs }}) is above the recommended range
            (30-50). This will increase computation time without significant improvement in results
            for most datasets.
          </PlAlert>
          <PlAlert
            v-if="app.model.args.nNeighbors > 5 && app.model.args.nNeighbors < 10"
            type="warn"
          >
            <template #title>Low Neighbor Count</template>
            The selected number of neighbors ({{ app.model.args.nNeighbors }}) is below the
            recommended range (10-30). This may result in fragmented clusters and loss of global
            structure.
          </PlAlert>
          <PlAlert v-if="app.model.args.nNeighbors > 30" type="warn">
            <template #title>High Neighbor Count</template>
            The selected number of neighbors ({{ app.model.args.nNeighbors }}) is above the
            recommended range (10-30). This may over-smooth the visualization and obscure
            fine-grained cell type distinctions.
          </PlAlert>
        </PlAccordionSection>
      </template>
    </GraphMaker>
